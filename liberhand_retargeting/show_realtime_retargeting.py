...
def main(
    retargeting_type: RetargetingType, hand_type: HandType, camera_path: Optional[str] = None
):
    """
    ...
    if retargeting_type == RetargetingType.dexpilot:
        config_name = f"liberhand_{hand_type_str}_dexpilot.yml"
    else:
        config_name = f"liberhand_{hand_type_str}.yml"
    config_path = f"configs/{config_name}"
    print(config_path)
    
    robot_dir = f"../configs/liberhand_description/urdf"
